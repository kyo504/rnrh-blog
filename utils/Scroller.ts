...
export default class Scroller {
    ...
    handlers: ScrollViewProps = {
        onContentSizeChange: () => {
            this.entriesCoordinates = Object.entries(this.entriesMap).sort((a, b) => a[1] - b[1]);
        },
        onScroll: ({ nativeEvent }) => {
            const offsetY = nativeEvent.contentOffset.y - this.offset + MIN_DIST_FROM_TOP_EDG;
            ...
            if (Platform.OS !== "android" || Math.abs(nativeEvent.velocity!.y) < 1) {
                for (let i = 0; i < this.entriesCoordinates.length; i++) {
                    ...
                    const upperBound =
                        i < this.entriesCoordinates.length - 1 ? this.entriesCoordinates[i + 1][1]
    ...
    removeSelecttedEntryListener(listener: (entryName: string) => void) {
    ...
    scrollToEntry(entryName: string) {
        console.log('scrollToEntry >>>>', entryName, this.entriesMap);
